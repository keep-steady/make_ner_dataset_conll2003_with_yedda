...
#          'a O\n',
    ...
    filterList = re.findall(entityRe, newSent)  #  find '[@~*]' 
    newSentLength = len(newSent)  # get the sentence length 
    chunk_list = []
    start_pos = 0
    end_pos = 0
    ...
    if len(filterList) == 0:
        ...
        # chunk_list : [sentence, 0, length of sentence, False]
    ...
    # filterList : ['[@AlphaBay Market#Company*]', '[@stolen customer account funds#Attack_Objective*]']
        ...
        for pattern in filterList:
            ...
            # chunk_list : [pattern, start position of pattern, end position of pattern, True]
    ...
    for eachList in full_list:
        ...
        # eachList[3] : True or False, with annotation or not
        ...
        if eachList[3]:
            ...
            # '[$', '[@' convert, there are errors cause cut '$' in tagging
            contLabelList = eachList[0].strip('[@').rsplit('#', 1)
                
            # the length of splited, error
            if len(contLabelList) != 2:
                print("Error: sentence format error!")
            # 'Company*' , make label removing '*'
            # label : Company
            label = contLabelList[1].strip('*]')

            # split when the annotation token consists of 2 annotation, 'AlphaBay Market' -> ['AlphaBay', 'Market']
            # nltk word tokenize
            if seget_nltk_tokenize:

                word_tokenized = word_tokenize(contLabelList[0])
                
                ## 'U.S.', 'U.S, .'가 있는 인덱스를 찾고, 그 뒤가 '.' 이면
                ## 합친다!!!
                check_word = ['U.S.', 'U.K.']
